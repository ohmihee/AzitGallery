...
let deal_post = async (req, res) => {
  ...
  try {
    ...
    console.log(userIdx, creator, userAddress, "useridx====");
    let data = {};
    let orderdata = [];

    let sellerId = await ItemInfo.findOne({
      where: {
        item_id: item_id,
      },
    });
    const buyerId = await User.findOne({
      where: {
        kaikas_address: userAddress,
      },
    });
    console.log(buyerId.user_idx, "buyerid===2");

    let result4 = await Orders.create({
      total_price: price,
      order_date: new Date(),
      final_order_state: "배송준비중",
      buyer: buyerId.user_idx,
      order_num: null,
      user_idx: sellerId.creator,
    });

    for (let i = 1; i <= qty; i++) {
      let result1 = await ItemDetail.findOne({
        where: { item_info_idx: item_id, size: size, color: color },
      });
      let result2 = await Nft.findOne({
        where: { nft_img_idx: result1.nft_idx, product_status: "판매중" },
      });
    ...
    // 판매완료 업데이트
    let item_detail_data = await ItemDetail.findOne({
      where: { item_info_idx: item_id, size: size, color: color },
    });
    let nft_idx_data = item_detail_data.nft_idx;
    let ch = await Nft.findAll({
      where: { nft_img_idx: nft_idx_data, product_status: "판매중" },
    });
    if (ch.length == 0) {
    ...
    ch2 = await ItemDetail.findAll({
      where: { item_info_idx: item_id, product_status: "판매중" },
    });
    if (ch2.length == 0) {
