  ...
  headers: [
    {
      ...
      value:
        "Basic " +
        Buffer.from(
          "KASKX0EA08C6X4X2EJ2VYJJB" +
            ":" +
            "vilV-HLyneRjPvTElJrHI4LmBezq5GynX3JIHr36"
        ).toString("base64"),
    },
...
let mint_nft_post = async (req, res) => {
  ...
  const keyring = caver.wallet.keyring.createFromPrivateKey(
    "0x97fb5a14f64d2d0e7791b7cce82fe58ed340c0400ee816956797b4d00b45730d"
  );
  ...
  try {
    ...
    let item_detail_idx = await ItemDetail.findAll({
      where: { item_info_idx: add_to_item_info.dataValues.item_id },
      attributes: ["nft_idx", "qty"],
    });
    ...
    for (let i = 0; i < item_detail_idx_arr.length; i++) {
      await ItemDetail.update(
        {
          item_code: `${add_to_item_info.dataValues.item_code}${last_digits_for_detail_arr[i]}`,
        },
        { where: { nft_idx: item_detail_idx_arr[i] } }
      );
    }
  ...
  async function getNFT(
    ...
    let privateKey =
      "0x97fb5a14f64d2d0e7791b7cce82fe58ed340c0400ee816956797b4d00b45730d"; // DB에서 가져와야 함
...
let KIP7Token_transfer = async () => {
  ...
  const kip7Instance = new caver.kct.kip7(
    "0xbd929FED827F26E84ca8b66A35Ef694F5829f9De"
  );
  ...
  let senderPrivateKey =
    "0x97fb5a14f64d2d0e7791b7cce82fe58ed340c0400ee816956797b4d00b45730d";
  const senderKeyring =
    caver.wallet.keyring.createFromPrivateKey(senderPrivateKey);
  // wallet에 keyring이 추가되지 않은 경우에만 keyring을 추가합니다.
