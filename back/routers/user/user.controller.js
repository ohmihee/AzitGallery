...
let selleradmin_access = async (req, res) => {
  let key = Object.keys(req.body);
  let keyObject = JSON.parse(key);
  let result = await Seller.update(
    { admin_approval: 3 },
    { where: { user_idx: keyObject } }
  );
};
...
let ship_update = async (req, res) => {
  ...
  try {
    ...
    let creator = await ItemInfo.findOne({
      where: { item_id: order_detail_num.item_id },
    });
    let useraddress = await User.findOne({
      where: { user_idx: creator.creator },
    });
